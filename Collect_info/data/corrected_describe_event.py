...
def build_description(row: dict, file_desc_map: dict) -> str:
    """
    Format aligné sur le dataset d'entraînement :

    file    → filename, TYPE, /dir, opened with App, ai_desc
    app     → App, used by the user to verb title
    dir     → directory, navigated by the user
    command → command, executed in terminal, cmddesc[, in /dir][, with file]
    """
    ...
    # ── APP ─────────────────────────────────
        ...
        if title and app:
            return f"{app}, used by the user to {verb} {title}"
        elif app:
            return f"{app}, used by the user"
    ...
    # ── COMMAND ─────────────────────────────
        ...
        parts = [command, "executed in terminal"]
        if cmd_desc and cmd_desc.strip() not in ("", "No description found"):
            # Retirer une éventuelle virgule ou espace résiduel en début
            cmd_desc_clean = cmd_desc.lstrip(", ").strip()
            if cmd_desc_clean:
                parts.append(cmd_desc_clean)
        for d in cmd_dirs:
            parts.append(f"in {d}")
        for f in cmd_files:
            parts.append(f"with the {f} file")

        return ", ".join(parts) if command else ""
